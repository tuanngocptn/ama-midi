...
test.describe('Collaboration', () => {
  ...
  test('owner can see created song under All Songs tab', async ({ page }) => {
    ...
    await page.getByPlaceholder('My awesome MIDI').fill('My Song');
    await page.getByRole('button', { name: 'Create', exact: true }).click();
    await expect(page.getByText('My Song')).toBeVisible({ timeout: 5000 });
